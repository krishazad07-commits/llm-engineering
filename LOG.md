...
## Day 24 (Sep 22) — TOP_K=10 sanity run: matches 15, ship 10

**What changed:** Reduced TOP_K from 15 → 10 in `eval_generation.py`. Added TOP_K to `RESULTS_PATH` filename so runs don't overwrite each other. Reordered constant definitions (TOP_K must precede RESULTS_PATH since the f-string interpolates it). Re-ran the full 50-question generation eval.
